...
public sealed class InstallationService : IInstallationService
{
    ...
    public InstallationService(IInstallationRepository installationRepository)
    {
        _installationRepository =
            ...
            ?? throw new ArgumentNullException(nameof(installationRepository));
    }

    public async Task<IReadOnlyList<InstallationDto>> GetAllAsync(
    ...
    public async Task<InstallationDto?> GetByIdAsync(
        Guid id,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<InstallationDto?> FindAsync(
        string version,
    ...
    )
    {
        ...
        var installation = await _installationRepository.FindAsync(
            version,
            variant,
            cancellationToken
        );
    ...
    public async Task<InstallationDto?> GetDefaultAsync(
        CancellationToken cancellationToken = default
    )
    {
    ...
    )
    {
        ...
        return _installationRepository.AddAsync(
            InstallationMapper.ToEntity(installation),
            cancellationToken
        );
    }
    ...
    )
    {
        ...
        return _installationRepository.UpdateAsync(
            InstallationMapper.ToEntity(installation),
            cancellationToken
        );
    }
